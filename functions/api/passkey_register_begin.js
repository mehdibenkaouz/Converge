...
export async function onRequest(context) {
  try {
    ...
    if (user) {
      // se ha già credenziale -> nickname preso
      ...
      for (let i = 0; i < 8; i++) {
        ...
        try {
          ...
          if (msg.includes("users.referral_code")) continue; // collisione, riprova
      ...
      user = await DB.prepare(`SELECT id, referral_code FROM users WHERE username = ? LIMIT 1`)
        .bind(nickname).first();
